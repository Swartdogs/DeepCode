...
double PIDControl::Calculate(double input)
{											// Calculate output based on input
	...
	{ // Limit output to within Minimum-Maximum range
		output = m_outputMax;
		limit += "OutMax ";
	}
	...
	{ // Ramping disabled or completed
		if (fabs(output) < m_outputLowErrorMin)
		{ // Keep output >= Low Error minimum
	...
	{ // Show Output
		if (limit.length() > 1)
			limit = limit.substr(0, limit.length() - 1);
...
void PIDControl::SetSetpoint(double setpoint, double inputNow, bool resetPID)
{
	...
	if (resetPID)
	{						   // Reset PID parameters (Default = True)
