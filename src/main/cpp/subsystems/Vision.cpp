...
void Vision::AddDistanceOffset(double offset) {                         // Add offset to the Target Distance
  m_targetDistance += offset;                                           //   Offset = Distance from camera-to-hand
...
int Vision::GetContoursFound() {                                        // Return # of Contours found
  return m_contoursFound;
}

Vision::SearchState Vision::GetSearchState() {                          // Return current Search State
  return m_searchState;
}

double Vision::GetTargetAngle() {                                       // Return Angle to target
  return m_targetAngle;
}

double Vision::GetTargetDistance() {                                    // Return Distance to target
  return m_targetDistance;
}

Vision::TargetSelect Vision::GetTargetSelect() {                        // Return target selected
   return m_targetSelect;
}

double Vision::GetTravelDistance() {                                    // Return Distance travelled during
   return m_travelDistance;                                             // image analysis
}
...
bool Vision::InTargetMode() {                                           // Return True if Camera is in Target Mode
  return(m_cameraMode == cmTarget);
}

void Vision::NewCameraMode(CameraMode mode) {                           // Used by Configure Camera thread to set
  m_cameraMode = mode;                                                  // mode after configuration is completed
}

void Vision::SearchResults(SearchState state, double targetAngle, double targetDistance, int contoursFound) {
  std::lock_guard<std::mutex> guard(myMutex);                           // Used by Target Search thread to report
  m_searchState       = state;                                          // search results
...
void Vision::SetCameraMode(CameraMode mode) {                           // Set Camera Mode (Use seperate thread)
...
void Vision::ConfigureCamera(Vision* host, cs::UsbCamera* myCamera, Vision::CameraMode mode) {
  if (mode == cmTarget) {                                               // Configure Camera for Target Search
      myCamera->SetBrightness(0);
      myCamera->SetExposureManual(10);
  } else {                                                              // Configure Camera for use by Driver
...
void Vision::TargetSearch(Vision* host, Vision::TargetSelect targetSelect) {     
  ...
  if(host->GetCameraImage(imageA)) {
      ...
      if (tapes.size() > 0) {                                           // Tapes Found
        ...
        for (int i = 0; i < tapes.size(); i++) {                        // Iterate thru tapes looking for Targets
          ...
          if (tapes[i].angle > 0){                                      // Based on angle, tape is a left tape
            ...
            visionTarget target;                                        // Determine Target Angle and Distance
            target.angle = atan2(targetCenter, 2.11) * 180 / 3.14159;
            target.distance = 4937 / (double)targetWidth;

            targets.push_back(target);                                  // Save Target Data in vector
          }
        }
    ...
    if (targetFound) {                                                  // Post search results
