...
void CmdDriveToTarget::End() {
  ...
  if (action.length() > 0) {
      if (m_distance <= 0) {
        ...
        sprintf(Robot::message, "Vision:   Drive %s  Distance Left=%5.1f  Right = %5.1f  Heading=%5.1f  Wait=%d msec", 
        ...
        sprintf(Robot::message, "Vision:   Drive %s  Distance Left=%5.1f  Right = %5.1f  Heading=%5.1f", action.c_str(),
